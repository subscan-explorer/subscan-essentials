...
import (
	...
	"github.com/itering/subscan/utiles/pusher/dingding"
)
...
func Subscribe() {
	var err error
	go dingding.DingClient.Push("alert", util.NetworkNode, "Subscribe daemon start")
	...
	util.SetEndpointCache(util.WSEndPoint)
	...
	subscribeOnce.Do(func() {
		...
		if err = SubscribeConn.WriteMessage(websocket.TextMessage, rpc.StateSubscribeStorage(103, subscribeStorageList)); err != nil {
			log.Info("write: %s", err)
		}
	})
